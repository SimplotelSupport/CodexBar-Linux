...
/// Stub for browser-cookie extraction. Alpha (v0.1.0) ships without the
/// browser-cookie auto-import path; users provide a session key via env var
/// or paste a manual cookie header. Always returns `Err`.
fn get_cookie_header(_domain: &str) -> Result<String, &'static str> {
    Err("browser cookie auto-import is not enabled in this build")
}
